...
class AppManager:
    ...
    def _set_active_tab(self, label):
        for lbl, btn in self._nav_btns.items():
            btn.config(bg="#1a252f" if lbl == label else "#2c3e50",
                       relief="sunken" if lbl == label else "flat")
